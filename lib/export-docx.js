...
const TABLE_W = 10464; // largura total da tabela (rótulo + colunas de dia), mantida do layout original

/** Calcula a largura das colunas de dia para N dias selecionados (1 a 5) */
function buildColWidths(numDias) {
  const n = numDias > 0 ? numDias : 4;
  const remaining = TABLE_W - COL_LABEL;
  const base = Math.floor(remaining / n);
  const dayWidths = new Array(n).fill(base);
  dayWidths[n - 1] += remaining - base * n; // resto do arredondamento na última coluna
  return dayWidths;
}

function row5(label, vals, dayWidths, opts) {
  opts = opts || {};
  const d = vals || [];
  return (
    "<w:tr>" +
    ...
    dayWidths
      .map(function (w, i) {
        return tc(d[i] || "", w, { size: 17, fill: opts.dayFill });
      })
      .join("") +
    "</w:tr>"
  );
...
function rowMerged(label, content, dayWidths, opts) {
  opts = opts || {};
  const span = dayWidths.length;
  const width = dayWidths.reduce(function (a, b) {
    return a + b;
  }, 0);
  return (
    "<w:tr>" +
...
function buildDocumentXml(plano) {
  ...
  const diasSemanaPlano =
    Array.isArray(plano.diasSemana) && plano.diasSemana.length
      ? plano.diasSemana
      : [
  ...
  const ordem = diasSemanaPlano.map(function (d) {
    return d.id;
  });
  const dayWidths = buildColWidths(ordem.length);
  const COLS = [COL_LABEL].concat(dayWidths);
  ...
  const table =
    ...
    "</w:tblGrid>" +
    header +
    row5("LANÇAMENTO DIÁRIO", pick("lancamento"), dayWidths, { dayFill: "F2F2F2" }) +
    rowMerged(
      "ENTRADA",
      plano.entrada ||
    ...
    row5("PLANEJAMENTO", planejamentoComCasa, dayWidths) +
    rowMerged(
      "LANCHE",
    ...
    row5("COORDENAÇÃO MOTORA FINA", pick("motoraFina"), dayWidths) +
    row5("PÁTIO / PSICOMOTRICIDADE", pick("patio"), dayWidths, { dayFill: "F2F2F2" }) +
    rowMerged(
      "SAÍDA",
      plano.saida ||
    ...
    row5("AJUDANTE DO DIA", pick("ajudante"), dayWidths) +
    rowMerged(
