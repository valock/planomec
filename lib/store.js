...
// import() dinâmico (não require) — confirmado em produção que o require()
// de um pacote com "exports" condicional não resolve dentro do bundle da
// function, enquanto o import() ESM resolve normalmente. O import só
// acontece se realmente rodando no Netlify — node server.js local nunca
// precisa desse pacote instalado.
let blobsModPromise = null;
function loadBlobsMod() {
  if (!blobsModPromise) blobsModPromise = import("@netlify/blobs");
  return blobsModPromise;
}

async function getBlobsStore(name) {
  const mod = await loadBlobsMod();
  return mod.getStore({ name: name, consistency: "strong" });
}

function blobsBackend(name) {
  return {
    async get(key) {
      const store = await getBlobsStore(name);
